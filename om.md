...
- 装置通过交换机连接服务器 装置地址(10.16.4.12) 交换机网关是10.16.4.254，服务器地址是199.1.1.10 首先装置与交换机同网段，装置到服务器需要配置路由

```
#只单条指向这个 IP
route add -net 199.1.1.10 netmask 255.255.255.255 gw 10.16.4.254 dev net3
```

- 同一网段设备可以直接二层互通; 不同网段必须依靠三层设备(网关机)转发数据包，路由就是告诉设备"陌生IP该发给谁转交"
- 网关机是三层设备，具备两大能力: 多网段接口(一边连接装置，一边连接服务器)，路由表查表转发:收到装置发来的包，拆开二层帧，读取三层目的IP,
  查询自身路由表，找到去往服务端IP网段的出接口与下一跳，重新封装二层帧，往服务器方向发送
- 装置是小区住户，只认识小区门卫（网关机）；住户不知道外地地址怎么走，信件全部丢给门卫；门卫有完整全国路线表，负责转运送到外地收件人（服务器）。
